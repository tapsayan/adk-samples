...
def create() -> None:
    ...
    logging.info("Creating AdkApp wrapper for the agent...")
    ...
    logging.info("Deploying agent with display name: %s", root_agent.name)
...
def delete(resource_id: str) -> None:
    ...
    logging.info("Attempting to delete agent: %s", resource_id)
    remote_agent = agent_engines.get(resource_id)
    remote_agent.delete(force=True)
    logging.info("Successfully deleted remote agent: %s", resource_id)


def list_agents() -> None:
    """Lists all deployed agent engines in the project and location."""
    logging.info("Listing all deployed agents...")
...
def main(argv: list[str]) -> None:
    ...
    logging.info("Using Staging Bucket: gs://%s", bucket)

    if not all([project_id, location, bucket]):
        logging.error(
            "Missing required configuration. Please set GOOGLE_CLOUD_PROJECT_ID, "
